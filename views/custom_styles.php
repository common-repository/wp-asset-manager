...
$(document).ready(function(){
    $("#sortable").sortable({
        stop : function(event, ui){
          ...
          $.ajax({
            url: '/wp-content/plugins/wp-asset-manager/reorder-styles.php',
            type : 'POST',
            data : { order : datastr },
...
if (isset($_GET['id'])){
  ...
  $i= 1; foreach ($all_styles as $style): 
  ...
  $_clean_array = serialize($update_styles);

  update_option('_wp_custom_style',$_clean_array);

  echo "<script>window.top.location = '/wp-admin/admin.php?page=custom-styles';</script>"; exit;
  ...
  <table class="wp-list-table widefat fixed pages" cellspacing="0">
    <thead>
      <tr>
        ...
        <th width="200">Styles</th>
        <th width="10">Action</th>
      </tr>
    </thead>

    <tbody id="sortable">
      <?php $ii = 1; foreach ($all_styles as $style): ?>
      <tr valign="center">
        <td class="drag"><img src="/wp-content/plugins/wp-asset-manager/reorder.png" border="0"/></td>
        <td>
            <label for="custom_src"><a href="<?php echo $style; ?>" target="_blank"><?php echo $style; ?></a></label>
            <input type="hidden" name="source_order" value="<?php echo $style; ?>"/>
        </td>
        <td>
            <a class="btn" href="/wp-admin/admin.php?page=custom-styles&id=<?php echo $ii; ?>">Remove</a>
        </td>        
      </tr>
    <?php $ii++; endforeach; ?>
    </tbody>
